...
const roundToNearest1point25 = (n) => Math.round(n / 1.25) * 1.25;
...
function ladderPlan({ dailyMax, increment, startRepCap = 12, isPullup = false, bodyweight = 80 }) {
  ...
  const rounder = isPullup ? roundToNearest1point25 : roundToNearest2point5;

  const targetTop = dailyMax;

  let startReps;
  let startWeight;

  // --- THIS ENTIRE BLOCK WAS MISSING IN MY PREVIOUS RESPONSE ---
  if (isPullup) {
    const maxRungsFromZero = Math.floor(1 + targetTop / increment);
  ...
  while (reps >= 1 && w <= targetTop + 1e-6 && guard < 400) {
    sets.push({ weight: rounder(w), reps });
    reps -= 1;
    w += increment;
    guard++;
  }
  ...
  if (!last || Math.abs(last.weight - targetTop) > 2.5 || last.reps !== 1) {
    if (!last || targetTop > last.weight) sets.push({ weight: rounder(targetTop), reps: 1 });
  }

  const tonnage = round2(sets.reduce((sum, s) => sum + s.weight * s.reps, 0));
  if (isPullup) sets.forEach((s) => (s.external = rounder(s.weight - bodyweight)));

  return {
    sets,
    tonnage,
    start: { weight: rounder(startWeight), reps: startReps },
  };
}

function rpPercentPlan({ dailyMax, isPullup = false }) {
  if (!dailyMax) return { sets: [], tonnage: 0, heavyReps: 0 };
  const rounder = isPullup ? roundToNearest1point25 : roundToNearest2point5;

  const scheme = [
    { pct: 0.9, reps: "1+" }, { pct: 0.85, reps: "2+" }, { pct: 0.8, reps: "3+" },
    { pct: 0.75, reps: "4+" }, { pct: 0.7, reps: "5+" },
  ];
  const sets = scheme.map((s) => ({
    weight: rounder(s.pct * dailyMax),
    reps: s.reps,
...
function rpFixedDropPlan({ dailyMax, drop = 10, sets = 4, repScheme = [3, 5, 7, 9], isPullup = false }) {
  if (!dailyMax) return { topSingle: null, sets: [], tonnage: 0 };
  const rounder = isPullup ? roundToNearest1point25 : roundToNearest2point5;

  const topSingle = rounder(0.95 * dailyMax);
  let w = topSingle;
  const rows = [];
  for (let i = 0; i < sets; i++) {
    ...
    rows.push({ weight: rounder(w), reps, note: `−${drop} kg` });
  }
...
function Select({ label, value, setValue, options }) {
  return (
    <label className="flex flex-col gap-1 text-sm">
      <span className="text-xs text-gray-600">{label}</span>
      <select value={value} onChange={(e) => setValue(e.target.value)} className="border rounded-lg px-3 py-2">
...
function ExerciseCard({ name, defaults, bodyweight, setBodyweight }) {
  ...
  const [pullupInputMode, setPullupInputMode] = useState("total");
  ...
  const plan = useMemo(() => {
    ...
    if (mode === "RP %") {
      return { ...rpPercentPlan({ dailyMax: dmaxTotal, isPullup }), _totalForPullup: dmaxTotal };
    }
    if (mode === "RP Drop") {
      ...
      return { ...rpFixedDropPlan({ dailyMax: dmaxTotal, drop, sets: nSets, repScheme: scheme, isPullup }), _totalForPullup: dmaxTotal };
    }
    return null;
  ...
  const results = useMemo(() => {
        ...
        <div className="mt-3">
          ...
          <table className="w-full text-sm mt-2 border">
            <thead><tr className="bg-gray-50">
              <th className="p-2 text-left">Set</th><th className="p-2 text-left">Weight</th>
              <th className="p-2 text-left">Reps</th>
              {isPullup && <th className="p-2 text-left">External (+/−) kg</th>}
            </tr></thead>
            <tbody>{p.sets.map((s, i) => (
              <tr key={i} className="border-t">
                <td className="p-2">{i + 1}</td><td className="p-2">{s.weight}</td>
      ...
      return (
        <div className="mt-3">
          <table className="w-full text-sm mt-2 border">
            <thead><tr className="bg-gray-50">
              <th className="p-2 text-left">Set</th><th className="p-2 text-left">Weight</th>
              {isPullup && <th className="p-2 text-left">External (+/−)</th>}
              <th className="p-2 text-left">Reps</th><th className="p-2 text-left">%</th>
            </tr></thead>
            <tbody>{plan.sets.map((s, i) => (
              <tr key={i} className="border-t">
                <td className="p-2">{i + 1}</td><td className="p-2">{s.weight}</td>
                {isPullup && <td className="p-2">{rounder(s.weight - bw)}</td>}
                <td className="p-2">{s.reps}</td><td className="p-2">{s.note}</td>
              </tr>))}
            </tbody>
          </table>
          <div className="mt-2 text-sm font-medium">Tonnage: {plan.tonnage} kg</div>
    ...
    if (mode === "RP Drop") {
      ...
      return (
        <div className="mt-3">
          <div className="text-xs text-gray-600">Top single (~95%): {plan.topSingle} kg × 1{isPullup ? `  (external ${rounder(plan.topSingle - bw)} kg)` : ""}</div>
          <table className="w-full text-sm mt-2 border">
            <thead><tr className="bg-gray-50">
              <th className="p-2 text-left">Back-off</th><th className="p-2 text-left">Weight</th>
              {isPullup && <th className="p-2 text-left">External (+/−)</th>}
              <th className="p-2 text-left">Reps</th><th className="p-2 text-left">Note</th>
            </tr></thead>
            <tbody>{plan.sets.map((s, i) => (
              <tr key={i} className="border-t">
                <td className="p-2">{i + 1}</td><td className="p-2">{s.weight}</td>
    ...
    <div className="border rounded-2xl p-4 shadow-sm bg-white">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex gap-3 flex-wrap">
          <Select label="Mode" value={mode} setValue={setMode} options={[{ label: "Ladder", value: "Ladder" }, { label: "Reverse Pyramid %", value: "RP %" }, { label: "Reverse Pyramid Drop", value: "RP Drop" }]} />
          {isPullup && (<Select label="Pull-up input" value={pullupInputMode} setValue={setPullupInputMode} options={[{ label: "Total (BW ± load)", value: "total" }, { label: "External only (+/− kg)", value: "external" }]} />)}
          <NumberInput label={isPullup ? (pullupInputMode === "external" ? "External max (+/− kg)" : "Total max (kg)") : "Daily max / top single (kg)"} value={dailyMaxInput} setValue={setDailyMaxInput} step={0.5} min={isPullup && pullupInputMode === "external" ? -200 : 0} />
          ...
          {isPullup && setBodyweight && (<NumberInput label="Bodyweight (kg)" value={bodyweight} setValue={setBodyweight} step={0.5} />)}
        </div>
      </div>
      {mode === "RP Drop" && (<div className="mt-3 grid grid-cols-3 gap-3">
...
function ExerciseList() {
  ...
  return (<div className="grid gap-4">
    {LIFTS.map((name) => (<ExerciseCard key={name} name={name} defaults={DEFAULTS[name]} bodyweight={name === "Pull-up" ? bodyweight : null} setBodyweight={name === "Pull-up" ? setBodyweight : null} />))}
  </div>);
}
